...
1. **Eyes (read-only)** — `windows`, `observe`/`read`, `eyes`, `eyes_read`/`look`, `cdp_discover`, `wait_cdp`, `status`, `stats`, `clipboard_get`, `apps`, `files_list` (inside roots), `search` / `search_content` (Perplexity; `PERPLEXITY_API_KEY`), `scrape` / `files_convert` / `files_find` / `xlsx` / `read_url` / `docling` / `rag` / `git` / `gh_pr` / `tavily` / `exa` / `ddg` / `wiki` / `weather` / `hn` / `memory_*` / `recall` / `mail_list` / `todo` / `onenote` / `teams` / `notion` / `linear` / `contacts` / `whoami` / `disk` / `hash` / `tree` / `files_stat`, `browser_use` / `browser_use_start` (Browser Use Cloud; `BROWSER_USE_API_KEY`), `ego` (status only), `lease_*`, `help`. No lease required.
