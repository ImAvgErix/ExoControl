...
1. **Eyes (read-only)** — `windows`, `observe`/`read`, `eyes`, `eyes_read`/`look`, `cdp_discover`, `wait_cdp`, `status`, `stats`, `clipboard_get`, `apps`, `files_list` (inside roots), `search` / `search_content` (Perplexity; `PERPLEXITY_API_KEY`), `lease_*`, `help`. No lease required.
