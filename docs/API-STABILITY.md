...
2.2 is additive: `search` / `search_web` / `search_content` (Perplexity), `browser_use` / `browser_use_start` / `browser_use_stop` (Browser Use Cloud), and `ego` (honest status). Existing op names stay. Remote CDP is still denied except Browser Use (API key) or `EXO_ALLOW_REMOTE_CDP=1`.
