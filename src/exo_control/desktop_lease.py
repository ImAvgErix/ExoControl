...
from exo_control.paths import lock_dir as _lock_dir
from exo_control.paths import state_dir as _state_dir
...
def acquire(agent_id: str, task: str = "", ttl_sec: float = 120) -> Dict[str, Any]:
    from exo_control.policy import max_lease_ttl_sec

    agent_id = str(agent_id or "").strip()
    task = str(task or "").strip() or "unspecified"
    if not agent_id:
        return {"ok": False, "error": "agent_id required"}
    cap = max_lease_ttl_sec()
    ttl_sec = max(0.05, min(float(ttl_sec), cap))
    with _exclusive():
        now = time.time()
        lease = _load_unlocked()
        if _active(lease, now):
            # Same holder may renew in-place (retry / reconnect). Foreign holder = conflict.
            if str(lease.get("agent_id") or "") == agent_id:
                expires = now + ttl_sec
                lease["expires_at"] = expires
                if task and task != "unspecified":
                    lease["task"] = task
        ...
        return {"ok": True, "token": token, "expires_at": _iso(expires), "ttl_sec": ttl_sec}
...
def renew(token: str, ttl_sec: float = 120) -> Dict[str, Any]:
    ...
    ttl_sec = max(0.05, min(float(ttl_sec), max_lease_ttl_sec()))
    with _exclusive():
        now = time.time()
        lease = _load_unlocked()
...
def force_release(token: Optional[str] = None, agent_id: Optional[str] = None) -> Dict[str, Any]:
    """Clear a lease by token, holder agent_id, or expired state.

    Unconditional wipe requires operator env ``EXO_ALLOW_FORCE_RELEASE=1``.
    """
    from exo_control.policy import allow_force_release
...
def status() -> Dict[str, Any]:
    with _exclusive():
        ...
        return {
            ...
            "task": lease.get("task"),
            "has_token": bool(lease.get("token")),
