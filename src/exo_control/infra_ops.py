...
# Fail closed: unmatched PID is denied (see kill_proc).
...
def is_protected_process(name: str) -> bool:
    low = (name or "").lower()
    stem = low.rsplit(".", 1)[0]
    for s in PROTECTED_PROCESS_SUBSTR:
        if len(s) <= 3:
            if stem == s or low.startswith(s + ".") or low.startswith(s + "-"):
                return True
        elif s in low:
            return True
    return False
...
def find_pids_by_name(name: str, max_hits: int = 20) -> List[Dict[str, Any]]:
    ...
    for row in listed.get("procs") or []:
        ...
        plow = pname.lower()
        # Short needles are exact / prefix only so "note" does not kill notepad.
        if len(needle) < 4:
            stem = plow.rsplit(".", 1)[0]
            if stem != needle and plow != needle and not plow.startswith(needle + "."):
                continue
        elif needle not in plow:
            continue
...
def env_get(name: Optional[str] = None) -> Dict[str, Any]:
    if name:
        ...
        from exo_control.policy import redact_env_value

        return redact_env_value(name, os.environ.get(name, ""))
    # names only
