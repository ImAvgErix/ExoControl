"""MarkItDown — local Office/PDF/text to markdown (allowrooted).

Uses ``markitdown`` when installed. Plain text/json/csv/html work without it.
"""
...
def convert(step: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not path:
        return {"ok": False, "error": "files_convert requires path", "code": "MISSING_PATH"}
    ok, resolved, outside = _resolve_under_roots(path)
    if not ok:
        return {"ok": False, "error": resolved, "code": "BAD_PATH"}
    if outside:
        denied = _outside_denied("files_convert", resolved, parse_confirm(step.get("confirm", False)))
    ...
    try:
        if _CONVERT is not None:
            ...
            if markdown is None:
                markdown = _try_markitdown(p)
        if markdown is None:
            return {
                "ok": False,
                "error": "files_convert needs markitdown for this type (pip install 'markitdown[all]')",
